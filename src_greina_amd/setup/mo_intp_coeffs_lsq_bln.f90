...
!!$OMP            delx,dely,fx,fy,fxx,fyy,fxy,jecp,nverts)
  DO jb = i_startblk, nblks_c
    ...
    !    and save normalized distance vectors z_dist_g/z_norm for future
    !    projection of gradient into edge-normal direction (secon part only needed for MUSCL)
      DO js = 1, ptr_ncells(jc,jb)
    ...
    !    reconstruction.
      ...
      !
        ...
        DO jec=1,nverts
          ...
          ! I. x^1y^0
          fx(jec) = distxy_v(jecp,1)**2._wp &
          &       + distxy_v(jecp,1)*distxy_v(jec,1) &
          &       + distxy_v(jec ,1)**2._wp

          ! II. x^0y^1
          fy(jec) = distxy_v(jecp,2)**2._wp &
          &       + distxy_v(jecp,2)*distxy_v(jec,2) &
          &       + distxy_v(jec,2)**2._wp

          IF ( lsq_dim_unk > 2 ) THEN

            ! III. x^2y^0
            fxx(jec) = (distxy_v(jecp,1)        + distxy_v(jec,1)       ) &
            &        * (distxy_v(jecp,1)**2._wp + distxy_v(jec,1)**2._wp)

            ! IV. x^0y^2
            fyy(jec) = (distxy_v(jecp,2)        + distxy_v(jec,2)       ) &
            &        * (distxy_v(jecp,2)**2._wp + distxy_v(jec,2)**2._wp)

            ! V. x^1y^1
            fxy(jec) = distxy_v(jecp,2) * (3._wp*distxy_v(jecp,1)**2._wp                         &
            &        + 2._wp * distxy_v(jecp,1) * distxy_v(jec,1) + distxy_v(jec,1)**2._wp )     &
            &        + distxy_v(jec,2) * ( distxy_v(jecp,1)**2._wp                               &
            &        + 2._wp * distxy_v(jecp,1) * distxy_v(jec,1) + 3._wp*distxy_v(jec,1)**2._wp )
