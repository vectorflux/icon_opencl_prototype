...
#if defined(STANDALONE) && defined(DATA_REGION)
  USE mo_data
#endif
...
!-------------------------------------------------------------------------
    ...
    DO jg = 1,n_dom
      ...
      IF(p_pe == p_io) THEN
        ...
        ENDIF
#else
#if defined(DATA_REGION)
         no_cells = cell
         no_verts = vertex
#else
        print *, "NOT SUPPORTED IN STANDALONE VERSION"
#endif
#endif
      ENDIF
...
#else
      write(*,*) "TOPOGRAPHY not supported in STANDALONE mode"
#endif
...
!    CALL finish(TRIM(routine),'End of topo-test implementation.')
  ...
  !-------------------------------------------------------------------------
    ...
    ENDDO
#else
    write (*,*) "read_netcdf_data not supported in STANDALONE mode"
#endif
