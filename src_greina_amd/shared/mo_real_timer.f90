...
#endif
  USE mo_parallel_ctl, ONLY: num_test_procs, num_work_procs, &
                             p_test_run,                     &
...
!$OMP END PARALLEL
  ...
  SUBROUTINE report(it)
    ...
    IF (tid == -1) THEN
      WRITE (message_text,'(a22,i6,4a12,f10.3)') '    '//srt(it)%text, &
           rt(it)%call_n, min_str, avg_str, max_str, tot_str, total
      CALL message ('',message_text,all_print=.TRUE.)
    ELSE
      WRITE (message_text,'(i2,a20,i6,4a12,f10.3)') tid,': '//srt(it)%text, &
           rt(it)%call_n, min_str, avg_str, max_str, tot_str, total
      CALL message ('',message_text,all_print=.TRUE.)
    ENDIF
