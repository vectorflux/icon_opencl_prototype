...
USE mo_mpi,                ONLY: p_pe, p_nprocs, p_bcast, p_sum, p_max, p_min, &
                                 p_send, p_recv
USE mo_parallel_ctl,       ONLY: p_test_pe, p_test_run, p_comm_work,           &
                                 n_ghost_rows, l_log_checks, l_fast_sum,       &
                                 p_pe_work, p_comm_work_test, p_work_pe0
USE mo_communication,      ONLY: exchange_data, exchange_data_hydro_c,         &
...
!modules interface-------------------------------------------
!subroutines
PUBLIC :: sync_patch_array, sync_patch_hydro_prog, check_patch_array, &
...
!>
!! Does boundary exchange for prognostic variables of the hydrostatic.
!!
!! Does boundary exchange for prognostic variables of the hydrostatic
!! dynamical core
!!
!! @par Revision History
!! Optimized version by Guenther Zaengl, Dec 2009, based on routines
!! developed by Rainer Johanni
!!
SUBROUTINE sync_patch_hydro_prog(p_patch, psfc, temp, vn)

!

   TYPE(t_patch), INTENT(IN) :: p_patch

   REAL(wp), INTENT(INOUT) :: psfc(:,:), temp(:,:,:), vn(:,:,:)
...
!-----------------------------------------------------------------------
   ...
   IF (p_test_run) THEN
     ...
     ALLOCATE(arr3(UBOUND(psfc,1), 1, UBOUND(psfc,2)))
     arr3(:,1,:) = psfc(:,:)
     CALL check_patch_array_3(SYNC_C, p_patch, arr3, 'sync')
     psfc(:,:) = arr3(:,1,:)
     DEALLOCATE(arr3)
     CALL check_patch_array_3(SYNC_C, p_patch, temp, 'sync')
     CALL check_patch_array_3(SYNC_E, p_patch, vn,   'sync')

   ENDIF

   ! Boundary exchange for work PEs
   IF(p_nprocs /= 1 .AND. p_pe /= p_test_pe) THEN
     CALL exchange_data_hydro_c(p_patch%comm_pat_c, psfc, temp)
     CALL exchange_data(p_patch%comm_pat_e, vn)
   ENDIF

END SUBROUTINE sync_patch_hydro_prog
...
!-----------------------------------------------------------------------
   ...
   IF (p_test_run) THEN
     ...
     IF (PRESENT(f3din6)) CALL check_patch_array_3(typ, p_patch, f3din6, 'sync')
   ENDIF

   ! Boundary exchange for work PEs
   IF(p_nprocs /= 1 .AND. p_pe /= p_test_pe) THEN
     IF (PRESENT(f4din)) THEN
       IF (nfields/=UBOUND(f4din,4)) &
     ...
     ELSE IF (PRESENT(f3din6)) THEN
       IF (nfields<6) CALL finish('sync_patch_array_mult','inconsistent arguments')
       ndim2tot = SIZE(f3din1,2)+SIZE(f3din2,2)+SIZE(f3din3,2)+ &
                  SIZE(f3din4,2)+SIZE(f3din5,2)+SIZE(f3din6,2)
       CALL exchange_data_mult(p_pat, nfields, ndim2tot, recv1=f3din1, recv2=f3din2, &
                               recv3=f3din3, recv4=f3din4, recv5=f3din5, recv6=f3din6)
