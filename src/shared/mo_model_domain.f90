...
TYPE t_grid_edges
  ...
  ! from cell c1 to cell c2 (c2-c2) goes outside the sphere
...
TYPE t_patch_ocean
  ...
  !!             intermediate z-coordinate surfaces. This data is provided by the user,
  ...
  TYPE(t_cartesian_coordinates), ALLOCATABLE :: edge2cell_coeff_t(:,:,:)
  ...
  ! Distance from vertex1 to vertex2 via dual edge midpoint
  REAL(wp), ALLOCATABLE :: dist_vert2vert(:,:)
...
!--------------------------------------------------------------------
  INTEGER :: global_max_childdom=1
  INTEGER, PARAMETER :: n_dom=1
  INTEGER :: n_dom_start=1
  ! the grid to use
  TYPE(t_patch),    TARGET :: global_patch
  TYPE(t_int_state),TARGET  :: global_patch_int_state
  TYPE(t_patch)     :: global_patch_array(n_dom)
  TYPE(t_int_state) :: global_int_state_array(n_dom)
