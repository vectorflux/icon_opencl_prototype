...
!-----------------------------------------------------------------------
  ...
  IF (n_ghost_rows<1) THEN
    CALL finish('mo_parallel_ctl:setup_parallel_run', &
  ...
  IF (p_test_run) THEN
    CALL message('mo_parallel_ctl:setup_parallel_run', &
...
!-----------------------------------------------------------------------
  ...
  CALL message('mo_parallel_ctl:setup_parallel_run', message_text)
...
END SUBROUTINE setup_parallel_run

END MODULE mo_parallel_ctl
