...
!STANDALONE (Fourestey) utilities

SUBROUTINE value_real(str,rnum,ios)

! Converts number string to a double precision real number

CHARACTER(len=*)::str
REAL(wp)    :: rnum
INTEGER :: ios, ipos, ilen
...
SUBROUTINE read_variable(handle, var, val)
implicit none
...
CHARACTER(len=200), dimension(10)  :: args
INTEGER                            :: nargs
...
DO WHILE (.true.)
        ...
        WRITE(6,*) 'looking for ', var
    READ (handle, '(A)', end=999) text
        WRITE(6,*) "->", text

    CALL parse(text, ' = ', args, nargs)

    IF (nargs.gt.0) then
!        WRITE(6,*) args(1)
    IF (args(1).eq.var) then
         !WRITE(6,*) args(1), " is ", args(2)
         CALL value_int(args(2), val, ierr)
         WRITE(6,*) args(1), " is ", val
         return
     ENDIF
     ENDIF
ENDDO
...
SUBROUTINE read_real_array(handle, var, arrayout)
implicit none

REAL(wp), intent(out), dimension(:,:) :: arrayout
CHARACTER(len = *), intent(in)  :: var
INTEGER, intent(in) :: handle


CHARACTER(len=200), dimension(100) :: args
CHARACTER(len=200) :: text
INTEGER                            :: nargs
...
REAL(wp), dimension(size(arrayout,1)*size(arrayout,2)) :: array
...
DO WHILE (.true.)
    ...
    !READ (handle, '(A)', end=999) text
    !READ(handle, '(A)', IOSTAT=stat) text 
    !if (IS_IOSTAT_END(stat)) then 
    !    write(6,*) 'rewinding 1'
    !    if (isrewound) goto 999
    !    isrewound = .true.
    !    rewind(handle)
    !endif 
...
!        DO WHILE (.true.)
        ...
        !WRITE(6,*) text
        CALL parse(text, '=', args, nargs)
        !WRITE(6,*) text, ' - > ', args(1:nargs), nargs
        !WRITE(6,*)  "\n\n"
        IF (nargs.gt.0) then
            !WRITE(6, *) args(1), var
            IF (args(1).eq.var) then
                IF (nargs.ge.1) then
                    ...
                    CALL parse(text, ', ', args, nargs)
                    DO i = 1, nargs
                        CALL value_real(args(i), array(count), ierr)
                        count = count + 1
                    ENDDO
                    DO WHILE (count <= size(array)) 
                        READ(handle, '(A)', end=999) text
                        CALL parse(text, ', ', args, nargs)
                        DO i = 1, nargs
                            CALL value_real(args(i), array(count), ierr)
...
arrayout = reshape(array,(/size(arrayout,1), size(arrayout,2)/))

END SUBROUTINE read_real_array



SUBROUTINE read_int_array(handle, var, arrayout)
implicit none

INTEGER, intent(out), dimension(:,:) :: arrayout
CHARACTER(len = *), intent(in)  :: var
INTEGER, intent(in)             :: handle
...
INTEGER, dimension(size(arrayout,1)*size(arrayout,2)) :: array 
        ...
        !WRITE(6, *) args(1), var
                IF (args(1).eq.var) then
                        IF (nargs.ge.1) then
                                ...
                                CALL parse(text, ', ', args, nargs)
                                DO i = 1, nargs
                                        CALL value_int(args(i), array(count), ierr)
                                        count = count + 1
                                ENDDO
                                DO WHILE (count <= size(array)) 
                                        READ(handle, '(A)', end=999) text
...
CALL set_data(patch_file)   ! Use the same file name for compatibility

IF (i_cell_type == 3) THEN ! triangular grid
   p_patch%n_patch_cells = cell
   p_patch%n_patch_verts = vertex
ELSEIF (i_cell_type == 6) THEN ! hexagonal grid
   p_patch%n_patch_cells = vertex
   p_patch%n_patch_verts = cell
ENDIF
p_patch%n_patch_edges = edge
#else
   open(99, file=TRIM(patch_file))
IF (i_cell_type == 3) THEN ! triangular grid
   call read_variable(99, 'cell', p_patch%n_patch_cells)
   call read_variable(99, 'vertex', p_patch%n_patch_verts);
ELSEIF (i_cell_type == 6) THEN ! hexagonal grid
   call read_variable(99, 'cell', p_patch%n_patch_verts)
   call read_variable(99, 'vertex', p_patch%n_patch_cells);
ENDIF
call read_variable(99, 'edge', p_patch%n_patch_edges);
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1) = cell_index
#else
   call read_int_array(99, 'cell_index', array_c_int(:,1:1))
#endif
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1) = cell_index
#else
    call read_int_array(99, 'cell_index', array_v_int(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:3) = neighbor_cell_index
#else
   call read_int_array(99, 'neighbor_cell_index', array_c_int(:,1:3))
#endif
#endif
   DO ji = 1, 3
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:3) = neighbor_cell_index
#else
   call read_int_array(99, 'neighbor_cell_index', array_v_int(:,1:3))
#endif
#endif
   DO ji = 1, 3
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:3) = edge_of_cell
#else
   call read_int_array(99, 'edge_of_cell', array_c_int(:,1:3))
#endif
#endif
   DO ji = 1, 3
     CALL reshape_idx( array_c_int(:,ji), p_patch%nblks_c, p_patch%npromz_c,  &
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:3) = edge_of_cell
#else
    call read_int_array(99, 'edge_of_cell', array_v_int(:,1:3))    
#endif
#endif
   DO ji = 1, 3
     CALL reshape_idx( array_v_int(:,ji), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:3) = vertex_of_cell
#else
   call read_int_array(99, 'vertex_of_cell', array_c_int(:,1:3)) 
#endif
#endif
   DO ji = 1, 3
     CALL reshape_idx( array_c_int(:,ji), p_patch%nblks_c, p_patch%npromz_c,  &
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:3) = vertex_of_cell
#else
   call read_int_array(99, 'vertex_of_cell', array_v_int(:,1:3))
#endif
#endif
   DO ji = 1, 3
     CALL reshape_idx( array_v_int(:,ji), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:3) = orientation_of_normal
#else
   call read_int_array(99,'orientation_of_normal',array_c_int(:,1:3))
#endif
#endif
   DO ji = 1, 3
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:3) = orientation_of_normal
#else
   call read_int_array(99,'orientation_of_normal',array_v_int(:,1:3))
#endif
#endif
   DO ji = 1, 3
...
#else
#if defined(DATA_REGION)
   array_c_real(:,1) = lon_cell_centre
#else
   call read_real_array(99, 'lon_cell_centre', array_c_real(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
   array_v_real(:,1) = lon_cell_centre
#else
   call read_real_array(99,  'lon_cell_centre', array_v_real(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
   array_c_real(:,1) = lat_cell_centre
#else
   call read_real_array(99, 'lat_cell_centre', array_c_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_c_real(:,1), p_patch%nblks_c, p_patch%npromz_c,  &
     &                p_patch%cells%center(:,:)%lat )
...
#else
#if defined(DATA_REGION)
   array_v_real(:,1) = lat_cell_centre
#else
   call read_real_array(99, 'lat_cell_centre', array_v_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_v_real(:,1), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_real(:,1) = cell_area_p
#else
        call read_real_array(99,'cell_area_p', array_c_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_c_real(:,1), p_patch%nblks_c, p_patch%npromz_c,  &
     &                p_patch%cells%area(:,:) )
...
#else
#if defined(DATA_REGION)
   array_v_real(:,1) = cell_area_p
#else
   call read_real_array(99,'cell_area_p', array_v_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_v_real(:,1), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_indlist(:,:) = start_idx_c
#else
   call read_int_array(99, 'start_idx_c', array_c_indlist(:,:))
#endif
#endif
...
#else
#if defined(DATA_REGION)
  array_v_indlist(:,:) = start_idx_c
#else
  call read_int_array(99, 'start_idx_c', array_v_indlist(:,:))
#endif
#endif
...
#else
#if defined(DATA_REGION)
  array_c_indlist(:,:) = end_idx_c
#else
  call read_int_array(99,  'end_idx_c', array_c_indlist(:,:))
#endif
#endif
  CALL reshape_idx_list( array_c_indlist,                                &
...
#else
#if defined(DATA_REGION)
  array_v_indlist(:,:) = end_idx_c
#else
  call read_int_array(99,  'end_idx_c', array_v_indlist(:,:))
#endif
#endif
  CALL reshape_idx_list( array_v_indlist,                                &
...
#else
#if defined(DATA_REGION)
   array_e_int(:,1) = edge_index
#else
call read_int_array(99, 'edge_index', array_e_int(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
array_e_int(:,1:2) = adjacent_cell_of_edge
#else
call read_int_array(99, 'adjacent_cell_of_edge',  array_e_int(:,1:2))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
  DO ji = 1, 2
...
#else
#if defined(DATA_REGION)
array_e_int(:,1:2) = edge_vertices
#else
call read_int_array(99, 'edge_vertices', array_e_int(:,1:2))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
  DO ji = 1, 2
...
#else
#if defined(DATA_REGION)
array_e_int(:,1) = edge_system_orientation
#else
call read_int_array(99, 'edge_system_orientation', array_e_int(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = lon_edge_centre
#else
call read_real_array(99, 'lon_edge_centre', array_e_real(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = lat_edge_centre
#else
call read_real_array(99, 'lat_edge_centre', array_e_real(:,1:1))
#endif
#endif
CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
  &                p_patch%edges%center(:,:)%lat )
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = zonal_normal_primal_edge
#else
call read_real_array(99, 'zonal_normal_primal_edge',array_e_real(:,1:1))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = meridional_normal_primal_edge
#else
call read_real_array(99, 'meridional_normal_primal_edge',array_e_real(:,1:1))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = zonal_normal_dual_edge
#else
call read_real_array(99, 'zonal_normal_dual_edge', array_e_real(:,1:1))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = meridional_normal_dual_edge
#else
call read_real_array(99, 'meridional_normal_dual_edge',array_e_real(:,1:1))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = edge_length
#else
call read_real_array(99, 'edge_length',  array_e_real(:,1:1))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
...
#else
#if defined(DATA_REGION)
array_e_real(:,1) = dual_edge_length
#else
call read_real_array(99, 'dual_edge_length', array_e_real(:,1:1))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   CALL reshape_real( array_e_real(:,1), p_patch%nblks_e, p_patch%npromz_e,  &
...
#else
#if defined(DATA_REGION)
array_e_real(:,1:2) = edge_vert_distance
#else
call read_real_array(99, 'edge_vert_distance', array_e_real(:,1:2))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   DO ji = 1, 2
...
#else
#if defined(DATA_REGION)
array_e_real(:,1:2) = edge_cell_distance
#else
call read_real_array(99, 'edge_cell_distance', array_e_real(:,1:2))
#endif
#endif
IF (i_cell_type == 3) THEN ! triangular grid
   DO ji = 1, 2
...
#else
#if defined(DATA_REGION)
array_e_int(:,1) = refin_e_ctrl
#else
call read_int_array(99, 'refin_e_ctrl',  array_e_int(:,1:1))
#endif
#endif
...
#else
#if defined(DATA_REGION)
array_e_indlist(:,:) = start_idx_e
#else
call read_int_array(99, 'start_idx_e', array_e_indlist(:,:))
#endif
#endif
...
#else
#if defined(DATA_REGION)
array_e_indlist(:,:) = end_idx_e
#else
call read_int_array(99, 'end_idx_e', array_e_indlist(:,:))
#endif
#endif
CALL reshape_idx_list( array_e_indlist,                                &
...
#else
#if defined(DATA_REGION)
array_v_int(:,1) = vertex_index
#else
call read_int_array(99, 'vertex_index', array_v_int(:,1:1))
#endif
#endif
   CALL reshape_idx( array_v_int(:,1), p_patch%nblks_v, p_patch%npromz_v,  &
     &               p_patch%verts%idx(:,:),  &
...
#else
#if defined(DATA_REGION)
array_c_int(:,1) = vertex_index
#else
call read_int_array(99,  'vertex_index', array_c_int(:,1:1))
#endif
#endif
   CALL reshape_idx( array_c_int(:,1), p_patch%nblks_c, p_patch%npromz_c,  &
     &               p_patch%cells%idx(:,:),  &
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:6) = vertices_of_vertex
#else
   call read_int_array(99,  'vertices_of_vertex', array_v_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_idx( array_v_int(:,ji), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:6) = vertices_of_vertex
#else
   call read_int_array(99, 'vertices_of_vertex', array_c_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_idx( array_c_int(:,ji), p_patch%nblks_c, p_patch%npromz_c,  &
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:6) = cells_of_vertex
#else
   call read_int_array(99, 'cells_of_vertex', array_v_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_idx( array_v_int(:,ji), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:6) = cells_of_vertex
#else
   call read_int_array(99, 'cells_of_vertex',  array_c_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_idx( array_c_int(:,ji), p_patch%nblks_c, p_patch%npromz_c,  &
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:6) = edges_of_vertex
#else
   call read_int_array(99, 'edges_of_vertex', array_v_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_idx( array_v_int(:,ji), p_patch%nblks_v, p_patch%npromz_v,  &
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:6) = edges_of_vertex
#else
   call read_int_array(99, 'edges_of_vertex', array_c_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_idx( array_c_int(:,ji), p_patch%nblks_c, p_patch%npromz_c,  &
...
#else
#if defined(DATA_REGION)
   array_v_int(:,1:6) = edge_orientation
#else
   call read_int_array(99, 'edge_orientation', array_v_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_real( REAL(array_v_int(:,ji),wp),            &
...
#else
#if defined(DATA_REGION)
   array_c_int(:,1:6) = edge_orientation
#else
   call read_int_array(99, 'edge_orientation', array_c_int(:,1:6))
#endif
#endif
   DO ji = 1, 6
     CALL reshape_real( REAL(array_c_int(:,ji),wp),            &
...
#else
#if defined(DATA_REGION)
   array_v_real(:,1) = longitude_vertices
#else
   call read_real_array(99, 'longitude_vertices', array_v_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_v_real(:,1), p_patch%nblks_v, p_patch%npromz_v,  &
     &                p_patch%verts%vertex(:,:)%lon )
...
#else
#if defined(DATA_REGION)
   array_c_real(:,1) = longitude_vertices
#else
   call read_real_array(99, 'longitude_vertices', array_c_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_c_real(:,1), p_patch%nblks_c, p_patch%npromz_c,  &
     &                p_patch%cells%center(:,:)%lon )
...
#else
#if defined(DATA_REGION)
   array_v_real(:,1) = latitude_vertices
#else
   call read_real_array(99, 'latitude_vertices', array_v_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_v_real(:,1), p_patch%nblks_v, p_patch%npromz_v,  &
     &                p_patch%verts%vertex(:,:)%lat )
...
#else
#if defined(DATA_REGION)
   array_c_real(:,1) = latitude_vertices
#else
   call read_real_array(99, 'latitude_vertices', array_c_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_c_real(:,1), p_patch%nblks_c, p_patch%npromz_c,  &
     &                p_patch%cells%center(:,:)%lat )
...
#else
#if defined(DATA_REGION)
   array_v_real(:,1) = dual_area_p
#else
   call read_real_array(99, 'dual_area_p', array_v_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_v_real(:,1), p_patch%nblks_v, p_patch%npromz_v,  &
     &                p_patch%verts%dual_area(:,:) )
...
#else
#if defined(DATA_REGION)
   array_c_real(:,1) = dual_area_p
#else
   call read_real_array(99, 'dual_area_p', array_c_real(:,1:1))
#endif
#endif
   CALL reshape_real( array_c_real(:,1), p_patch%nblks_c, p_patch%npromz_c,  &
     &                p_patch%cells%area(:,:) )
...
#else
#if defined(DATA_REGION)
  array_v_indlist(:,:) = start_idx_v
#else
  call read_int_array(99, 'start_idx_v', array_v_indlist(:,:))
#endif
#endif
  CALL reshape_idx_list( array_v_indlist,                                &
   &                    p_patch%verts%start_idx, p_patch%verts%start_blk )
...
#else
#if defined(DATA_REGION)
   array_c_indlist(:,:) = start_idx_v
#else
   call read_int_array(99, 'start_idx_v', array_c_indlist(:,:))
#endif
#endif
  CALL reshape_idx_list( array_c_indlist,                                &
   &                    p_patch%cells%start_idx, p_patch%cells%start_blk )
...
#else
#if defined(DATA_REGION)
  array_v_indlist(:,:) = end_idx_v
#else
  call read_int_array(99, 'end_idx_v', array_v_indlist(:,:))
#endif
#endif
  CALL reshape_idx_list( array_v_indlist,                                &
   &                    p_patch%verts%end_idx, p_patch%verts%end_blk )
...
#else
#if defined(DATA_REGION)
  array_c_indlist(:,:) = end_idx_v
#else
  call read_int_array(99, 'end_idx_v', array_c_indlist(:,:))
#endif
#endif
  CALL reshape_idx_list( array_c_indlist,                                &
   &                    p_patch%cells%end_idx, p_patch%cells%end_blk )
