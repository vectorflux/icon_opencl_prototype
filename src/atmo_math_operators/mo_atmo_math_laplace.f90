...
USE mo_intp_methods,        ONLY: t_int_state, edges2verts_scalar, verts2edges_scalar
USE mo_model_domain,        ONLY: t_patch
! USE mo_model_domain_import, ONLY: l_limited_area
USE mo_global_variables,    ONLY: nproma, i_cell_type, nlev
USE mo_exception,           ONLY: finish
USE mo_loopindices,         ONLY: get_indices_c, get_indices_e
! USE mo_sync,                ONLY: SYNC_C, SYNC_E, SYNC_V, sync_patch_array

USE mo_atmo_math_gradients
USE mo_atmo_math_divrot
...
REAL(wp) ::  &
  &  z_div_c(nproma,nlev,ptr_patch%nblks_c),  &
  &  z_rot_v(nproma,nlev,ptr_patch%nblks_v),  &
  &  z_rot_e(nproma,nlev,ptr_patch%nblks_e)
...
IF (PRESENT(opt_nabla2) ) THEN
  p_nabla2 => opt_nabla2
ELSE
  ALLOCATE (z_nabla2_vec_e(nproma,nlev,ptr_patch%nblks_e))
...
REAL(wp) :: &
  &  z_nabla2_vec_e(nproma,nlev,ptr_patch%nblks_e),  &
  &  z_nabla4_vec_e(nproma,nlev,ptr_patch%nblks_e)
...
REAL(wp) ::  &
  &  z_grad_fd_norm_e(nproma,nlev,ptr_patch%nblks_e)
...
REAL(wp), DIMENSION (nproma,nlev,ptr_patch%nblks_c) :: aux_c
...
IF (PRESENT(opt_nabla2) ) THEN
  p_nabla2 => opt_nabla2
ELSE
  ALLOCATE (z_nab2_c(nproma,nlev,ptr_patch%nblks_c))
  p_nabla2 => z_nab2_c
ENDIF
...
END MODULE mo_atmo_math_laplace
