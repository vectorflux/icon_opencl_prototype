...
!! grid in the nonhydrosatic model branch.
...
MODULE mo_vertical_grid
  ...
  USE mo_global_variables,    ONLY: nlev, nlevp1, ltestcase, nproma, i_cell_type,          &
                                    rayleigh_coeff, damp_height, igradp_method, ivctype,   &
                                    min_lay_thckn, top_height, msg_level,     &
                                    decay_scale_1, decay_scale_2, decay_exp, flat_height,  &
                                    vwind_offctr, exner_expol, l_zdiffu_t, thslp_zdiffu,   &
                                    thhgtd_zdiffu, msg_level
  USE mo_setup_parameters,    ONLY :setup_sleve
  USE mo_hyb_params,          ONLY: vct_a, vct_b, vct, read_hyb_params
  ...
  USE mo_parallel_ctl,        ONLY: p_test_run
  ...
  REAL(wp) :: layer_thickness=-999.0_wp        ! (m)
  INTEGER :: n_flat_level= 2

  INTEGER:: nflat, nrdmax, nflat_gradp(max_dom)

  PUBLIC :: set_nh_metrics, init_hybrid_coord, init_sleve_coord, nflat, nrdmax, nflat_gradp
  ...
  !!
  SUBROUTINE init_hybrid_coord

    REAL(wp) :: z_height, z_flat
    INTEGER  :: jk, ist

    CHARACTER(len=MAX_CHAR_LENGTH) :: dyn_option
    !-------------------------------------------------------------------------
...
!       write(0,*)"layer_thickness < 0.0_wp"

      ! call  in nonhydrostatic mode
      dyn_option = 'nonhydro'
!      CALL read_hyb_params (dyn_option) ! already done
...
!         write(0,*) nlevp1, jk
    ...
    ELSE
      ...
      nflat  = n_flat_level  !< copy testcase given value to nflat
      z_flat = REAL(nlevp1-nflat,wp) * layer_thickness
      DO jk = 1, nlevp1
...
!     STOP
   IF (msg_level > 10) & 
    CALL message('mo_vertical_grid: init_hybrid_coord', 'is done')
  ...
  SUBROUTINE init_sleve_coord
    ...
    z_exp = LOG(min_lay_thckn/top_height)/LOG(2._wp/pi*ACOS(REAL(nlev-1,wp)/REAL(nlev,wp)))
    ...
    DO jk = 1, nlevp1
      vct_a(jk)      = top_height*(2._wp/pi*ACOS(REAL(jk-1,wp)/REAL(nlev,wp)))**z_exp
      vct(jk)        = vct_a(jk)
    ...
    ! nflat must not be zero
    nflat = MAX(1,nflat)

   IF (msg_level > 10) & 
    CALL message('mo_vertical_grid: init_sleve_coord', 'is done')
  ...
  SUBROUTINE set_nh_metrics(p_patch, p_nh, p_int)
    ...
    INTEGER :: jg, jk, jk1, jk_start, jb, jc, je, jv, nlen, &
               nblks_c, npromz_c, nblks_e, npromz_e, nblks_v, npromz_v
    INTEGER :: i_startidx, i_endidx, i_startblk

    REAL(wp) :: z_diff, z1, z2, z3, z_help(nproma), z_maxhgt(nlevp1), z_temp(nproma), &
                z_rho(nproma,nlev), z_aux1(nproma), z_aux2(nproma)
    REAL(wp) :: z_fac1, z_fac2, z_topo_dev(nproma), z_maxslope, z_maxhdiff, z_offctr
    REAL(wp), ALLOCATABLE :: z_ifv(:,:,:), z_mfv(:,:,:), z_geo_e(:,:,:)
    REAL(wp), ALLOCATABLE :: z_me(:,:,:),z_maxslp(:,:,:),z_maxhgtd(:,:,:)
...
!$OMP DO PRIVATE(jb, nlen, jk, z_fac1, z_fac2, z_topo_dev)
      DO jb = 1,nblks_c
        ...
        DO jk = 1, nlev
          ! geopotential (main point)
          p_nh(jg)%metrics%geopot(1:nlen,jk,jb) = 0.5_wp * grav * &
          & (p_nh(jg)%metrics%z_ifc(1:nlen,jk  ,jb) &
...
!$OMP DO PRIVATE(jb, nlen, jk, z_fac1, z_fac2, z_topo_dev)
      DO jb = 1,nblks_v
        ...
        IF (ivctype == 1) THEN ! hybrid Gal-Chen coordinate
          DO jk = 1, nlev
            z_ifv(1:nlen,jk,jb) = vct_a(jk) + vct_b(jk)*z_ifv(1:nlen,nlevp1,jb)
          ENDDO
        ELSE IF (ivctype == 2) THEN ! SLEVE coordinate (Leuenberger et al. MWR 2010)

          DO jk = 1, nflat
            z_ifv(1:nlen,jk,jb) = vct_a(jk)
          ENDDO
          DO jk = nflat + 1, nlev
            ! Scaling factors for large-scale and small-scale topography
            z_fac1 = SINH((top_height/decay_scale_1)**decay_exp - &
...
!$OMP END PARALLEL
      ...
      ALLOCATE(z_geo_e(nproma,nlev,p_patch(jg)%nblks_e))

      IF (p_test_run) z_geo_e = 0._wp

      CALL cells2edges_scalar(p_nh(jg)%metrics%geopot, &
...
!$OMP DO PRIVATE(jb, nlen, jk, jc, z_diff)
      DO jb = 1,nblks_c
        ...
        DO jk = 1, nlevp1
          DO jc = 1, nlen
            z_diff = MAX(0.0_wp,p_nh(jg)%metrics%z_ifc(jc,jk,jb)-damp_height)
            p_nh(jg)%metrics%rayleigh_w(jc,jk,jb)= &
            &   rayleigh_coeff * (SIN ( pi_2 * z_diff &
            &   /(p_nh(jg)%metrics%z_ifc(jc,1,jb)-damp_height)))**2
          ENDDO
        ENDDO
      ENDDO
!$OMP END DO
...
!$OMP END PARALLEL
      ...
      nrdmax = 1
      DO jk = 2, nlevp1
        IF (z_maxhgt(jk) >= damp_height) nrdmax = jk-1
      ENDDO

      IF (msg_level > 10) THEN
        WRITE(message_text,'(a,i10)') 'End index of Rayleigh damping layer: ', nrdmax
...
!$OMP DO PRIVATE(jb, nlen, jk, z_help, z_temp, z_aux1, z_aux2, z_rho)
        DO jb = 1,nblks_c
          ...
          ! Half level fields are not needed at surface level => set to zero
          p_nh(jg)%metrics%theta_ref_ic(:,nlevp1,jb)      = 0._wp
          p_nh(jg)%metrics%d_exner_dz_ref_ic(:,nlevp1,jb) = 0._wp
...
!$OMP END PARALLEL
  ...
  SUBROUTINE prepare_zdiffu(p_patch, p_nh, p_int, maxslp, maxhgtd)
    ...
    REAL(wp),        INTENT(IN)        :: maxslp(nproma,nlev,p_patch%nblks_c)
    REAL(wp),        INTENT(IN)        :: maxhgtd(nproma,nlev,p_patch%nblks_c)
    ...
    REAL(wp) :: max_nbhgt, z_vintcoeff(nproma,nlev,p_patch%nblks_c,3)
    INTEGER  :: nbidx(nproma,nlev,p_patch%nblks_c,3)
